...
async def assign_evaluation_steps(
        ...
        buffered_instances:  str = Query(None, description="Optional language ID filter"),
    ...
    if buffered_instances:
        for x in buffered_instances.split(","):
            x = x.strip()
            try:
                if x:
                    buffer.append(uuid.UUID(x))
            except ValueError:
...
async def assign_ab_test_to_user(
        ...
        user_proficiency: int = Query(1, ge=1, le=10, description="User proficiency level (1-10)"),
        batch_size: int = Query(1, ge=1, le=5, description="batch size for buffering"),
        service: EvaluationService = Depends(get_evaluation_service),
        current_user: User = Depends(get_current_active_user)
):
...
async def submit_evaluation_step(
        ...
        step_id: UUID4 = Query(..., description="The evaluation step ID"),
    ...
    try:
        result = await service.submit_evaluation_step(
            ...
            abtest_decision=data.abtest_decision if data else None
        )
