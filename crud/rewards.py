...
class RewardsCRUD:
    """
    Class for basic CRUD operations on reward-related models.
    No business logic included here.
    """
    ...
    # User Milestone CRUD operations
    async def create_user_milestone(self, user_milestone_data: UserMilestoneCreate) -> UserMilestone:
    ...
    # Challenge Reward CRUD operations
    ...
    async def update_challenge_reward_raw(self, reward_id: uuid.UUID, reward_data: ChallengeRewardUpdate) -> Optional[
        ChallengeReward]:
        """Raw update without business logic checks"""
    ...
    async def list_challenge_rewards(self, skip: int = 0, limit: int = 100, reward_type: Optional[RewardType] = None) -> \
            List[ChallengeReward]:
        query = select(ChallengeReward)

        if reward_type:
            query = query.where(ChallengeReward.reward_type == reward_type)

        query = query.offset(skip).limit(limit)
        result = await self.db.execute(query)
        return list(result.scalars().all())

    # User Challenge Reward CRUD operations
    async def create_user_challenge_reward(self, award_data: UserChallengeRewardCreate) -> UserChallengeReward:
