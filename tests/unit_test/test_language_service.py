...
@pytest.fixture
def mock_inactive_language(mock_language):
    """Create a mock inactive Language object"""
    mock_language.is_active = False
    mock_language.deactivated_at = datetime.utcnow()
...
@pytest.mark.asyncio
async def test_get_all_languages_default(language_service, mock_language):
    """Test default behavior (active=True) returns only active languages"""
    # Setup
    mock_language.id = uuid.uuid4()
    mock_language.is_active = True
    language_service.language_repository.get_all.return_value = [mock_language]
    ...
    # Assert
    assert len(result) == 1
    assert result[0].id == str(mock_language.id)

    # Get the actual call arguments
    actual_args, actual_kwargs = language_service.language_repository.get_all.call_args

    # Verify positional arguments (skip, limit)
    assert actual_args == (0, 100)
...
@pytest.mark.asyncio
async def test_get_all_languages_inactive_only(language_service, mock_inactive_language):
    """Test inactive=True returns only inactive languages"""
    # Setup
    mock_inactive_language.id = uuid.uuid4()
    mock_inactive_language.is_active = False
    language_service.language_repository.get_all.return_value = [mock_inactive_language]

    # Execute
    result = await language_service.get_all_languages(inactive=True)

    # Assert
    assert len(result) == 1
    assert result[0].id == str(mock_inactive_language.id)
    # Get the actual call arguments
    actual_args, actual_kwargs = language_service.language_repository.get_all.call_args
...
@pytest.mark.asyncio
async def test_get_all_languages_all(language_service, mock_language, mock_inactive_language):
    """Test all_=True returns both active and inactive languages"""
    # Setup
    mock_language.id = uuid.uuid4()
    mock_inactive_language.id = uuid.uuid4()
    language_service.language_repository.get_all.return_value = [mock_language, mock_inactive_language]

    # Execute
    result = await language_service.get_all_languages(all_=True)
...
@pytest.mark.asyncio
async def test_get_all_languages_pagination(language_service, mock_language):
    """Test pagination parameters are properly passed"""
    # Setup
    language_service.language_repository.get_all.return_value = [mock_language]

    # Execute
    await language_service.get_all_languages(skip=10, limit=5)

    # Get the actual call arguments
    actual_args, actual_kwargs = language_service.language_repository.get_all.call_args

    # Verify positional arguments (skip, limit)
    assert actual_args == (10, 5)
...
@pytest.mark.asyncio
async def test_get_all_languages_parameter_conflict(language_service, mock_language):
    """Test parameter precedence (all_ overrides others)"""
    # Setup
    language_service.language_repository.get_all.return_value = [mock_language]

    # Execute with conflicting params
    await language_service.get_all_languages(active=True, inactive=True, all_=True)

    # Assert - all_ should take precedence
    # Get the actual call arguments
    actual_args, actual_kwargs = language_service.language_repository.get_all.call_args
...
@pytest.mark.asyncio
async def test_get_all_languages_id_conversion(language_service, mock_language, mock_inactive_language):
    """Test UUID IDs are properly converted to strings"""
    # Setup
    mock_language.id = uuid.uuid4()
    mock_inactive_language.id = uuid.uuid4()
    language_service.language_repository.get_all.return_value = [mock_language, mock_inactive_language]
    ...
    # Assert
    assert result[0].id == str(mock_language.id)
    assert result[1].id == str(mock_inactive_language.id)
